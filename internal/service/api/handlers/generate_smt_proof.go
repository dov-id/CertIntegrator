...
func GenerateSMTProof(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		Log(r).WithError(err).Debug("failed to parse generate proof request")
		ape.RenderErr(w, problems.BadRequest(err)...)
		return
	}

	contract, err := MasterQ(r).ContractsQ().FilterByAddresses(request.Data.Attributes.Contract).Get()
	if err != nil {
		Log(r).WithError(err).Debug("failed to get contract by address")
		ape.RenderErr(w, problems.InternalError())
		return
	}

	if contract == nil {
		Log(r).WithError(err).Debugf("no contract with address `%s`", request.Data.Attributes.Contract)
		ape.RenderErr(w, problems.NotFound())
		return
	}

	mTree, err := merkletree.NewMerkleTree(
		r.Context(),
		postgres.NewStorage(Cfg(r).DB().Clone(), contract.Id),
		data.MaxMTreeLevel,
	)
	if err != nil {
		Log(r).WithError(err).Debug("failed to open/create merkle tree")
		ape.RenderErr(w, problems.InternalError())
		return
	}
	...
	if err != nil {
		Log(r).WithError(err).Debug("failed to generate merkle tree proof")
		ape.RenderErr(w, problems.InternalError())
		return
	}

	if !proof.Existence {
		Log(r).Debugf("proof for `%s` key not found in MerkleTree", request.Data.Attributes.NodeKey)
	...
	if err != nil {
		Log(r).WithError(err).Debug("failed to convert key big int to hash")
		ape.RenderErr(w, problems.InternalError())
		return
	}
	...
	if err != nil {
		Log(r).WithError(err).Debug("failed to convert value big int to hash")
		ape.RenderErr(w, problems.InternalError())
		return
	}
